  ...
  <div className="fixed inset-0 bg-black/90 flex justify-center items-center z-50">
    <div className="bg-black text-white w-full max-w-lg p-6 rounded-xl shadow-xl relative overflow-y-auto max-h-[90vh]">
      ...
      <div className="space-y-4">
        ...
        {/* Shirt Quality */}
        <select value={shirtQuality} onChange={(e) => setShirtQuality(e.target.value)} className="w-full border border-gray-600 bg-zinc-900 px-4 py-2 rounded text-white">
        ...
        {/* Colors */}
        <select value={colors} onChange={(e) => setColors(e.target.value)} className="w-full border border-gray-600 bg-zinc-900 px-4 py-2 rounded text-white">
        ...
        {/* Sizes */}
        <select value={sizes} onChange={(e) => setSizes(e.target.value)} className="w-full border border-gray-600 bg-zinc-900 px-4 py-2 rounded text-white">
          ...
          {colorPreviews.length > 0 && (
            <div className="flex gap-2 mt-4 overflow-x-auto">
              {colorPreviews.map((src, i) => (
                <img key={i} src={src} alt={`color-${i}`} className="w-16 h-16 object-cover rounded border border-gray-600" />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="flex justify-end mt-6">
        <button onClick={handleSave} className="bg-red-600 hover:bg-red-700 px-6 py-2 rounded text-white font-semibold">Save</button>
      </div>
      <button onClick={() => setIsModalOpen(false)} className="absolute top-3 right-4 text-gray-500 text-xl">&times;</button>
    </div>
  </div>
)}
